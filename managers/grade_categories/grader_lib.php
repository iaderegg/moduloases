...
function update_category($id, $parent, $name, $aggregation,$weight){
    global $DB;
    ...
    $succes = $DB->update_record('grade_categories',$object);
...
//update_item(117,54,"Taller 1",1,true);
/**
              ...
              for ($j = 0; $j < count($report->tablecolumns); $j++) {
                  ...
                  if (isset($content)&&!isCategoryTotal($content)) {
                      if(isCategory($content)){
                        ...
                        $html .= "<$celltype id = 'cat_$categoryid' $headers class='$class' $colspan><div id = '$aggregation' class = 'agg'> $content<div id = '$name' title = '$name' hidden></div> <p style = 'display: inline' class = 'maxweight' id = '$maxweight'>$weight</p> <div id = 'buttons' style = 'float: right !important'><button title = 'Crear nuevo item o categoria' class = 'btn new' >+</button> <button title = 'Modificar Categoria' class = 'btn glyphicon glyphicon-pencil edit'></button></div></div></$celltype>\n";
                      }else{
                        ...
                        $html .= "<$celltype id = 'it_$id_item' $headers class='$class' $colspan>$content <p class = 'maxweight' style = 'display: inline'>$weight</p> <div style = 'float: right !important'><button title = 'Modificar Item' class = 'btn glyphicon glyphicon-pencil edit'></button></div></$celltype>\n";
                      }
                  }
              }
              $html .= "</tr>\n";
